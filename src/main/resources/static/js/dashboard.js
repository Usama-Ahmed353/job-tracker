...
//     // Find the nearest parent card element to remove it
//     const card = buttonEl.closest('[class*="rounded-2xl"]') || buttonEl.closest('[class*="border"]') || buttonEl.parentElement.parentElement;
//     if (card) card.remove();
...
//     });
//
//     // Update dashboard KPI stat cards
//     const totalEl = document.getElementById("kpiStatTotal");
//     const activeEl = document.getElementById("kpiStatActive");
//     const interviewEl = document.getElementById("kpiStatInterviews");
//     const offersEl = document.getElementById("kpiStatOffers");
//     const activeStatuses = ["APPLIED", "ASSESSMENT", "INTERVIEW"];
//     if (totalEl) totalEl.textContent = filteredApps.length;
//     if (activeEl) activeEl.textContent = filteredApps.filter(a => activeStatuses.includes(a.status)).length;
//     if (interviewEl) interviewEl.textContent = filteredApps.filter(a => a.status === "INTERVIEW").length;
//     if (offersEl) offersEl.textContent = filteredApps.filter(a => a.status === "OFFER").length;
// }
//
...
//
// function initApp() {
//     // Universal theme state initialization — dark-theme is the optional override
...
//     }
//
//     // Toggle active state CSS class for light mode styling
//     if (archiveBtn) {
//         if (showArchivedPoolGlobal) {
//             archiveBtn.classList.add("archive-active");
//         } else {
//             archiveBtn.classList.remove("archive-active");
//         }
//     }
//
...
// }
//
// // ---------- Theme Engine Management Engine ----------
// /**
//  * Toggles systemic document style class sheets layout definitions.
//  * Persists layout preference selections inside local state structures.
//  */
...
//     if (!body || !icon) return;
//
//     // Default is light; toggle adds/removes dark-theme class
...
//
// // ---------- Time Filter Pills ----------
// function setTimeFilter(period, btn) {
//     // Update active state on pills
//     document.querySelectorAll(".time-filter-btn").forEach(b => b.classList.remove("active"));
//     if (btn) btn.classList.add("active");
//     // The filter is cosmetic on the board view (applications don't have a UI date filter)
//     // Re-render board to reflect any future date-based filtering hooks
//     renderBoard();
// }
//
...
function toggleThemeMode() {
    ...
    // Charts pick their colors at render time based on the current theme class,
    // so if the Analytics view is open, re-render them immediately instead of
    // leaving stale colors until the next navigation/reload.
    const analyticsView = document.getElementById("analytics-view");
    if (analyticsView && analyticsView.classList.contains("active") && typeof loadAnalytics === "function") {
        loadAnalytics();
    }
}
